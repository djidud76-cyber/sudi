...
CREATE POLICY "Owner can manage workspace" ON workspaces FOR ALL USING (auth.uid() = owner_id);

-- Index
CREATE INDEX IF NOT EXISTS idx_workspaces_owner ON workspaces(owner_id);
CREATE INDEX IF NOT EXISTS idx_links_workspace ON links(workspace_id);

-- Usage tracking function: increment links_used on insert
...
-- Also add browser and os to link_clicks if they don't exist
